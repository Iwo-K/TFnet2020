...
########## DE ##########
    dir.create(file.path("./DESeq2_reports"), showWarnings = FALSE)

    #' Comparison of three guides (comparison using the condition factor)
    if(any(grepl("sg1", unique(tempmeta$conditionPlate)))){
        contrastsg1 = c("conditionPlate",
                        paste0(gene, "_sg1_", plate),
    ...
    if(any(grepl("sg2", unique(tempmeta$conditionPlate)))){
        contrastsg2 = c("conditionPlate",
                        paste0(gene, "_sg2_", plate),
                        paste0("control_", plate))
        print("Comparing:")
        print(contrastsg2)
    ...
    if(any(grepl("sg3", unique(tempmeta$conditionPlate)))){
        contrastsg3 = c("conditionPlate",
                        paste0(gene, "_sg3_", plate),
                        paste0("control_", plate))
        print("Comparing:")
        print(contrastsg3)
  ...
  #' Comparison of all sg vs control (comparison using the gene factor)
    ...
    de = runDESeq2(tempdata,  tempmeta, genedata,
                 ...
                 lfc_threshold = lfc_threshold,
                 save = save,
                 filename = "./DE/DESeq2_reports/DESeq2_report",
                 parallel = parallel)
