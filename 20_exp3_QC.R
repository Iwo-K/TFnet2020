#' # QC analysis of the TF interaction experiment (exp3) data (Kucinski et al. 2020)
#' Author: Iwo Kucinski
...
meta = read.csv("./data/TFnet_data/exp3_meta.csv", header = TRUE, as.is = TRUE, row.names = 1)
...
data = read.csv("./data/TFnet_data/exp3_exon_counts.csv", header = TRUE, as.is = TRUE, row.names = 1)
data = data[ ,meta$cellid]

#' ### Intronic counts
intron_data = read.csv("./data/TFnet_data/exp3_intron_counts.csv", header = TRUE, as.is = TRUE, row.names = 1)
...
dataQC = scQC(data, metadata = meta,
              ...
              tr_higeneno = 4000,
              tr_mitofrac = 0.05,
              filebase = "./QC_exp3/QCperplate")
metaQC = meta[colnames(dataQC),]
table(metaQC$condition)

#' Adding the QC statistic to metadata
qc = read.csv("./QC_exp3/QCperplatepercell.csv", row.names = 1, header = TRUE, as.is = TRUE)
qc = qc[metaQC$cellid,]
...
mapstats = mapstats[metaQC$cellid,]
...
write.csv(metaQC, "./procdata/exp3_meta_QC.csv")
write.csv(dataQC, "./procdata/exp3_counts_QC.csv")
