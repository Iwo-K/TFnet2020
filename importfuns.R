...
## Not in use anymore
## #' Function for reading in the featureCounts statistics
## get_stats = function(path, type = "star"){
##     require(data.table)

##     x = list.files(path, ".*fcounts\\.txt.summary", full.names = TRUE)
##     print(x)
...
##     if(type == "star") colnames(allstats) = gsub("(.*STARout\\/)(.*)(_Aligned.*)", "\\2", colnames(allstats))
##     else if(type == "subread") colnames(allstats) = gsub("(.*SAM\\/)(.*)(\\.sort\\.bam)", "\\2", colnames(allstats))
##     else print("type needs to be either star or subread")

##     allstats = as.data.frame(t(allstats))

##     allstats$Total_Reads = rowSums(allstats)
##     row.names(allstats) = gsub("-", ".", row.names(allstats))
##     return(allstats)
## }
